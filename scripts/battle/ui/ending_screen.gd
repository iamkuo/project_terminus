...
@onready var reward_label: Label = $BackgroundPanel/VBoxContainer/rewardlabel
...
func show_result(winning_team: int):
	...
	var rewards = BattleManager.calculate_rewards(winning_team)
	if reward_label:
		reward_label.text = "獲得獎勵: " + str(rewards.exp) + " EXP, " + str(rewards.crystals) + " 水晶"
	
	kills_label.text = "擊敗數: " + str(BattleManager.enemy_killed)
	damage_label.text = "造成傷害: " + str(int(BattleManager.damage_dealt))
	time_label.text = "戰鬥時間: " + str("%01d" % BattleManager.minutes) + "分" + str("%02d" % BattleManager.seconds) + "秒"
...
func _on_return_pressed():
	# Apply rewards and return to main world
